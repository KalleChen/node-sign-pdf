  ...
  async sign(pdfBuffer, p12Buffer, additionalOptions = {}) {
    ...
    if (options.tsa) {
      signer.unauthenticatedAttributes = [{
        type: _nodeForge.default.pki.oids.timeStampToken,
    ...
    if (options.tsa) {
      ...
      const token = await (0, _helpers.tsa)({
        tsaUrl: options.tsa,
        signature
      ...
      p7.signerInfos[0].value[6].value[0].value[1] = _nodeForge.default.asn1.create(_nodeForge.default.asn1.Class.UNIVERSAL, _nodeForge.default.asn1.Type.SET, true, [timestampToken]);
